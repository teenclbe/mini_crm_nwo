    ...
    <div class="container mx-auto p-5">
        ...
        <form action="{{ route('bookings.update', $booking) }}" method="POST" class="max-w-lg">
            ...
            <div class="mb-4">
                ...
                <input type="text" name="customer_phone" value="{{ old('customer_phone', $booking->customer_phone) }}"
                    class="mt-1 p-2 w-full border rounded" required>
            </div>
            <div class="mb-4">
                    ...
                    @foreach ($rooms as $room)
                        <div class="flex items-center">
                            <input type="checkbox" name="room_ids[]" value="{{ $room->id }}" id="room_{{ $room->id }}"
                                ...
                                data-price="{{ $room->price_per_hour }}" {{ in_array($room->id, old('room_ids', $booking->rooms->pluck('id')->toArray())) ? 'checked' : '' }}>
            ...
            <div class="mb-4">
                ...
                <input type="datetime-local" name="start_time" value="{{ old('start_time', $booking->start_time) }}"
                    class="mt-1 p-2 w-full border rounded" required>
            </div>
            <div class="mb-4">
                <label for="end_time" class="block text-sm font-medium text-gray-700">End Time</label>
                <input type="datetime-local" name="end_time" value="{{ old('end_time', $booking->end_time) }}"
                    class="mt-1 p-2 w-full border rounded" required>
            </div>
